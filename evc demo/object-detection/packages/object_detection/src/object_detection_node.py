...
from duckietown_msgs.msg import Twist2DStamped, EpisodeStart, BoolStamped
...
class ObjectDetectionNode(DTROS):
    def __init__(self, node_name):
        ...
        # publish whether a duckie is detected
        # if so, turning event would be triggered
        # see more in fsm package
        #shutdown_cmd_topic = f"/{self.veh}/shutdown"
        #self.pub_shutdown_cmd = rospy.Publisher(
        #    shutdown_cmd_topic, Empty, queue_size=1,
        #)
        self.pub_turning = rospy.Publisher(
            "~image/duckie_detected", 
            BoolStamped, 
            queue_size=1, 
        )
        ...
        self.v = rospy.get_param("~speed", 0.1)
        self.truning_v = 0
        #aido_eval = rospy.get_param("~AIDO_eval", False)
        #self.log(f"AIDO EVAL VAR: {aido_eval}")
        # remember to check olin's setting of aido_eval
    ...
    def image_cb(self, image_msg):
        ...
        # as soon as the duckie is detected, a message is sent to the finite state machine
        if detection:
            self.log("Duckie detected... shutting down")
            turning_msg = BoolStamped()
            turning_msg.header.stamp = image_msg.header
            turning_msg.data = True
            self.pub_turning.publish(turning_msg)
            #self.pub_shutdown_cmd.publish(Empty())

            
        else:
            self.log("log: no duckie")
        ...
        # if in the debug mode, the image with bbox would be shown in "~image/compressed" namespace.
        # actually in our pj we can just ignore this part
        if self._debug:
    ...
    # convert detection info into bool info    
    # In our pj this function returns true when the object is duckie (class0), 
    # and the score is bigger than 90,
    # and the duckie is close enough to the car. 
    # To change the settings of bbox, class and score, go to solution/integration_activity.py
    ...
    def pub_car_commands(self, stop, header):
        ...
        # omega>0 left, <0 right
        # let omega default = 0.5
        # -8 < omega < +8
        car_control_msg.omega = 0.0
        
        # the next line is commented because we don't control car from this node
