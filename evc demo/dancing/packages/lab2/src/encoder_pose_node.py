...
class EncoderPoseNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.log("DONE Loading kinematics calibration...")
    ...
    def cb_left_encoder(self, encoder_msg):
        ...
        if not (self.ODOMETRY_ACTIVITY or self.ACTIVITY):
            #self.log("left activity is not set, returning")
            return
        ...
        if self.left_tick_prev is None:
            ...
            #self.log("left not yet read ticks from the wheel, returning")
            return
    ...
    def cb_right_encoder(self, encoder_msg):
        ...
        # Do nothing if the activity is not set
        if not (self.ODOMETRY_ACTIVITY or self.ACTIVITY):
            #self.log("right activity is not set, returning")
            return
        ...
        if self.right_tick_prev is None:
            ...
            #self.log("right not yet read ticks from the wheel, returning")
            return
    ...
    def calculate_pose(self):
        ...
        self.log(f"θ = {np.rad2deg(self.theta_curr)} degree")
        self.log("")
        ...
        #self.log(self._state)

        #self.log(self.ACTIVITY and self._state is not None)
    ...
    def _do_state2(self):
        ...
        # Drive straight for 0.5 metres
        if self._state == _State.STATE2b:
            dist_threshold = 0.15
            drive = self.dist < 0.75 - dist_threshold
            if drive:
                v, omega = self.speed_gain, -0
        ...
        if self._state == _State.STATE2d:
            dist_threshold = 0.15
            drive = self.dist < 1.5 - dist_threshold
            if drive:
                v, omega = self.speed_gain, -0
        ...
        if self._state == _State.STATE2f:
            dist_threshold = 0.25
            drive = self.dist < 2.25 - dist_threshold
            if drive:
                v, omega = self.speed_gain, -0
    ...
    def _do_state3(self):
        ...
        if self._state == _State.STATE3b:
            dist_threshold = 0.4
            drive = self.dist < 1 - dist_threshold
            if drive:
                v, omega = self.speed_gain, -0
    ...
    def _do_state4(self):
        ...
        if self._exited_threshold :
            ...
            if 340 < np.rad2deg(self.theta_curr) < 20:
                self._set_state(_State.STATE4b)
            #if self.dist > (1.25 - dist_thresh) / 2:
            #    self._set_state(_State.STATE4b)
        elif self._state == _State.STATE4b:
            v, omega = 0.2, -1.0
            if 160 < np.rad2deg(self.theta_curr) < 200:
                self._set_state(_State.STATE4c)
        elif self._state == _State.STATE4c:
            v, omega = 0.2, -1.0
            if 70 < np.rad2deg(self.theta_curr) < 110:
                self._exited_threshold = True
